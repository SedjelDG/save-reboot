...
- Auto transcript uses `openai/whisper-tiny.en` by default. First use may download/load the model and take time; verify the transcript manually before saving the voice. Reader-made mic/system recordings and WAV uploads decode without ffmpeg; MP3/M4A/WebM uploads still require ffmpeg.
